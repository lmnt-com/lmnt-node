...
async function loadFs() {
  if (Runtime.detectRuntime() === 'node') {
    fs = await Promise.resolve().then(() => _interopRequireWildcard(require('fs')));
  }
}
const DEBUG = false;
const logDebug = (...args) => {
...
class StreamingSynthesisConnection {
  ...
  async _setupWebSocket() {
    this._socket = await (0, _simpleWebSocket.createSimpleWebSocket)(URL_STREAMING);
...
class Speech {
  ...
  async synthesize(text, voice, options = {}) {
    ...
    const fields = ['format', 'language', 'model', 'conversational', 'length', 'return_durations', 'return_seed', 'sample_rate', 'seed', 'speed'];
